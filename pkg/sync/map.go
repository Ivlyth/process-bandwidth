...
func newEntry[K comparable, V any](i V) *entry[V] {
...
func (m *Map[K, V]) Load(key K) (value V, ok bool) {
	...
	if !ok {
		return generic.ZeroValue[V](), false
	}
	return e.load()
}
...
func (m *Map[K, V]) LoadOrStore(key K, valueFn func() V) (actual V, loaded bool) {
	...
	read, _ := m.read.Load().(readOnly[K, V])
	if e, ok := read.m[key]; ok {
		actual, loaded, ok := e.tryLoadOrStore(valueFn)
		if ok {
	...
	if e, ok := read.m[key]; ok {
		...
		actual, loaded, _ = e.tryLoadOrStore(valueFn)
	} else if e, ok := m.dirty[key]; ok {
		actual, loaded, _ = e.tryLoadOrStore(valueFn)
		m.missLocked()
	} else {
...
func (e *entry[V]) tryLoadOrStore(fn func() V) (actual V, loaded, ok bool) {
	p := atomic.LoadPointer(&e.p)
	if p == expunged {
		return generic.ZeroValue[V](), false, false
	}
	if p != nil {
